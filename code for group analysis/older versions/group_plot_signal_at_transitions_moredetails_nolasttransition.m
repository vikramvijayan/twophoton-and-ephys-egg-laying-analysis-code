function [transition_structure, transition_structure_notperfly] = group_plot_signal_at_transitions_moredetails_nolasttransition(recordings_list, ROI_num, filter_out_chrimson_data, filter_out_egg_times, backsub)
% ROI_num = -1 use patch
...
for rec_index = 1:1:length(recordings_list)
    ...
    %wheelvelocity_noave = (pi*7*-25).*[0; diff(unwrap(recording.movie1.filtered_wheel))]./ (2*pi);
