...
function varargout = view_imaging_or_patching_5s_patch_config(varargin)
% VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG MATLAB code for view_imaging_or_patching_5s_patch_config.fig
%      VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG, by itself, creates a new VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG or raises the existing
%      singleton*.
%
%      H = VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG returns the handle to a new VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG or the handle to
%      the existing singleton*.
%
%      VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG.M with the given input arguments.
%
%      VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG('Property','Value',...) creates a new VIEW_IMAGING_OR_PATCHING_5S_PATCH_CONFIG or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before view_imaging_or_patching_5s_patch_config_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to view_imaging_or_patching_5s_patch_config_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help view_imaging_or_patching_5s_patch_config

% Last Modified by GUIDE v2.5 29-Oct-2020 15:01:51

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @view_imaging_or_patching_5s_patch_config_OpeningFcn, ...
    'gui_OutputFcn',  @view_imaging_or_patching_5s_patch_config_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before view_imaging_or_patching_5s_patch_config is made visible.
function view_imaging_or_patching_5s_patch_config_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to view_imaging_or_patching_5s_patch_config (see VARARGIN)
...
% Choose default command line output for view_imaging_or_patching_5s_patch_config
...
behavior = [];
behavior = [90029,90837,91316,92326,92831,97245,97245,97339];
...
% UIWAIT makes view_imaging_or_patching_5s_patch_config wait for user response (see UIRESUME)
end

% --- Outputs from this function are returned to the command line.
function varargout = view_imaging_or_patching_5s_patch_config_OutputFcn(hObject, eventdata, handles)
...
[~, closest_image_plus] = min(abs(handles.recording.movie1.time_stamps(ceil(handles.t_slider.Value))-handles.recording.tseries(handles.tseries_for_plot).Time_s+2.5));

[~, closest_image_minus] = min(abs(handles.recording.movie1.time_stamps(ceil(handles.t_slider.Value))-handles.recording.tseries(handles.tseries_for_plot).Time_s-2.5));
...
if(length(handles.size_mat) == 5)
    ...
    if(handles.mean_z.Value)
        curr_image = mean(handles.tsStack(:,:,1,closest_image_minus:closest_image_plus,:),5);
        curr_image = mean(curr_image(:,:,1,:),4);
    end
...
axes(handles.axes3);
if(handles.patch ==0)
    line([handles.recording.tseries(handles.tseries_for_plot).Time_s(closest_image),handles.recording.tseries(handles.tseries_for_plot).Time_s(closest_image)],[-100,100],'Color','m');
end
if(handles.patch==1)
...
axes(handles.axes5);
line([handles.recording.movie1.time_stamps(ceil(handles.t_slider.Value)),handles.recording.movie1.time_stamps(ceil(handles.t_slider.Value))], [-100,100],'Color','m');
...
MOVIE = VideoWriter([handles.recording.abfname(1:end-4) '_movie_5s_causal.avi']);
