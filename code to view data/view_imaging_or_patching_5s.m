...
function varargout = view_imaging_or_patching_5s(varargin)
% VIEW_IMAGING_OR_PATCHING_5S MATLAB code for view_imaging_or_patching_5s.fig
%      VIEW_IMAGING_OR_PATCHING_5S, by itself, creates a new VIEW_IMAGING_OR_PATCHING_5S or raises the existing
%      singleton*.
%
%      H = VIEW_IMAGING_OR_PATCHING_5S returns the handle to a new VIEW_IMAGING_OR_PATCHING_5S or the handle to
%      the existing singleton*.
%
%      VIEW_IMAGING_OR_PATCHING_5S('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIEW_IMAGING_OR_PATCHING_5S.M with the given input arguments.
%
%      VIEW_IMAGING_OR_PATCHING_5S('Property','Value',...) creates a new VIEW_IMAGING_OR_PATCHING_5S or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before view_imaging_or_patching_5s_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to view_imaging_or_patching_5s_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help view_imaging_or_patching_5s

% Last Modified by GUIDE v2.5 11-May-2020 08:27:27

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @view_imaging_or_patching_5s_OpeningFcn, ...
    'gui_OutputFcn',  @view_imaging_or_patching_5s_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before view_imaging_or_patching_5s is made visible.
function view_imaging_or_patching_5s_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to view_imaging_or_patching_5s (see VARARGIN)
...
% Choose default command line output for view_imaging_or_patching_5s
...
% UIWAIT makes view_imaging_or_patching_5s wait for user response (see UIRESUME)
end

% --- Outputs from this function are returned to the command line.
function varargout = view_imaging_or_patching_5s_OutputFcn(hObject, eventdata, handles)
...
MOVIE = VideoWriter([handles.recording.abfname(1:end-4) '_movie_5s.avi']);
