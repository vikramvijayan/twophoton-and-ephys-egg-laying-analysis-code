% min power and max power are the minimum and maximum power to use [other
% pulses are thrown out]

function [out] = plot_cross_correlations_pulse(recording, ROI_num, min_power, max_power)
...
for interate_pulses = 1:1:length(tmp_times)
    if(recording.abf.PWMlaser_uWpermm2(tmp_times(interate_pulses)) > min_power && recording.abf.PWMlaser_uWpermm2(tmp_times(interate_pulses)) < max_power)
        recording.abf.pulse_OFF_times = [recording.abf.pulse_ON_times ;tmp_times(interate_pulses)] ;
    end
end
